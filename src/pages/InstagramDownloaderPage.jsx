    ...
    const InstagramDownloaderPage = () => {
      ...
      const fetchTrending = async () => {
        ...
        if (dbError) {
          ...
          toast({ title: 'Error', description: 'Could not load trending videos.', variant: 'destructive' });
          setTrendingVideos([]);
        } else {
        ...
        >
          ...
          <TrendingVideos 
            ...
            onVideoSelect={(selectedUrl) => {
              if (isBatchMode) {
                toast({ title: 'Info', description: 'Switch to Single Download mode to use Trending Videos.', duration: 3000 });
              } else {
                setUrl(selectedUrl);
